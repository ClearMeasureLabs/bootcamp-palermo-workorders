...
internal static class ChatClientConfigValidator
{
    public static ChatClientAvailabilityResult Validate(ChatClientConfig config)
    {
        ...
        {
            return new ChatClientAvailabilityResult(true,
                $"Chat client is configured for local Ollama ({OllamaChatDefaults.DefaultModelId})");
        }
        ...
        if (missing.Count > 0)
        {
            return new ChatClientAvailabilityResult(false,
                $"Chat client is not configured. Azure OpenAI is partially configured. Set the following environment variables: {string.Join(", ", missing)}");
        }
