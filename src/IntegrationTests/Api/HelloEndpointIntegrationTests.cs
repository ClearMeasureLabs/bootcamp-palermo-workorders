...
public class HelloEndpointIntegrationTests
{
    ...
    public async Task Should_Return200WithoutApiKey_When_HelloAndMiddlewareEnabled()
    {
        ...
        using (var unversionedDocument = JsonDocument.Parse(await unversioned.Content.ReadAsStringAsync()))
        {
            unversionedDocument.RootElement.GetProperty("message").GetString().ShouldBe("Hello, World!");
        }

        var versioned = await client.GetAsync("/api/v1.0/hello");
        versioned.StatusCode.ShouldBe(HttpStatusCode.OK);
        using (var versionedDocument = JsonDocument.Parse(await versioned.Content.ReadAsStringAsync()))
        {
            versionedDocument.RootElement.GetProperty("message").GetString().ShouldBe("Hello, World!");
        }
    }
}
