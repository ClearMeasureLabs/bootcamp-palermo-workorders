...
public class HelloEndpointIntegrationTests
{
    ...
    [Test]
    public async Task Should_Return200AndJsonHello_When_GetUnversioned()
    {
    ...
    [Test]
    public async Task Should_Return200AndJsonHello_When_GetVersioned()
    {
    ...
    public async Task Should_Return200WithoutApiKey_When_HelloAndMiddlewareEnabled()
    {
        ...
        var unversionedPayload = await unversioned.Content.ReadFromJsonAsync<HelloResponse>(
            new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
        unversionedPayload!.Message.ShouldBe("Hello, World!");

        var versioned = await client.GetAsync("/api/v1.0/hello");
        versioned.StatusCode.ShouldBe(HttpStatusCode.OK);
        var versionedPayload = await versioned.Content.ReadFromJsonAsync<HelloResponse>(
            new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
