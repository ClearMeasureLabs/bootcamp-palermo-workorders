...
[TestFixture]
public class WorkRequestsBulkImportIntegrationTests
{
    private SqliteConnection? _sharedMemoryHold;
    private WorkRequestsBulkImportWebApplicationFactory? _factory;
    private HttpClient? _httpClient;

    [OneTimeSetUp]
    public void OneTimeSetUp()
    {
        _sharedMemoryHold = new SqliteConnection(WorkRequestsBulkImportWebApplicationFactory.SqliteConnectionString);
        _sharedMemoryHold.Open();
        _factory = new WorkRequestsBulkImportWebApplicationFactory();
    ...
    public async Task ShouldCreateDrafts_WhenCsvUploaded()
    {
        ...
        var response = await _httpClient!.PostAsync(new Uri(_httpClient.BaseAddress!, "api/v1.0/work-requests/bulk-import"), content);
        ...
        var count = await db2.Set<WorkRequest>().CountAsync(w => w.Creator!.UserName == "bulk-user");
        count.ShouldBe(2);
    }
