...
using DomainWorkRequest = ClearMeasure.Bootcamp.Core.Model.WorkRequest;
...
[TestFixture]
public class GrpcWorkRequestIntegrationTests
{
    ...
    public async Task Should_StartHost_When_GrpcConfigured()
    {
        ...
        var client = new WorkRequests.WorkRequestsClient(channel);
    ...
    public async Task Should_ReturnExpectedPayload_When_UnaryGrpcCallSucceeded()
    {
        using (var scope = _factory!.Services.CreateScope())
        {
            ...
            db.Add(creator);
            var order = new DomainWorkRequest
            {
                Number = "GRPC-001",
                Title = "Test title",
        ...
        using var channel = CreateGrpcChannel();
        var client = new WorkRequests.WorkRequestsClient(channel);
        var reply = await client.GetWorkRequestByNumberAsync(new GetWorkRequestByNumberRequest { Number = "GRPC-001" });
        reply.WorkRequest.ShouldNotBeNull();
        reply.WorkRequest.Number.ShouldBe("GRPC-001");
        reply.WorkRequest.Title.ShouldBe("Test title");
        reply.WorkRequest.Description.ShouldBe("Test description");
        reply.WorkRequest.RoomNumber.ShouldBe("101");
        reply.WorkRequest.StatusKey.ShouldBe(WorkRequestStatus.Draft.Key);
        reply.WorkRequest.CreatorUsername.ShouldBe("grpc-creator");
        reply.WorkRequest.CreatedDateUtc.ShouldNotBeNull();
    }
    ...
    public void Should_MapGrpcStatus_When_InvalidArgumentOrNotFound()
    {
        ...
        var emptyEx = Should.Throw<RpcException>(async () =>
            await client.GetWorkRequestByNumberAsync(new GetWorkRequestByNumberRequest { Number = "   " }));
        emptyEx.StatusCode.ShouldBe(StatusCode.InvalidArgument);

        var notFoundEx = Should.Throw<RpcException>(async () =>
            await client.GetWorkRequestByNumberAsync(new GetWorkRequestByNumberRequest { Number = "missing-wo-999" }));
    ...
    public async Task Should_ExcludeGrpcFromApiRateLimiter_When_DesignRequires()
    {
        ...
        var grpcClient = new WorkRequests.WorkRequestsClient(channel);
