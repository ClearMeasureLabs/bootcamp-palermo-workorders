...
public class McpWorkOrderToolTests
{
    ...
    {
        var employee = new Employee("user1", "John", "Doe", "john@test.com");
        var order = new WorkOrder { Creator = employee, Number = "WO-100", Title = "Test order", Description = "A description", Instructions = "Wear PPE", RoomNumber = "101" };
    ...
    public async Task ShouldCreateDraftWorkOrder()
    {
        ...
        var result = await WorkOrderTools.CreateWorkOrder(bus, numberGenerator, "New Work Order", "Fix the broken window", "creator1", instructions: "Lock out tag out");

        result.ShouldContain("New Work Order");
        result.ShouldContain("Fix the broken window");
        result.ShouldContain("Lock out tag out");
