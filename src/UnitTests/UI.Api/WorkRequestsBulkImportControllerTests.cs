...
[TestFixture]
public class WorkRequestsBulkImportControllerTests
{
    [Test]
    public async Task ShouldReturnBadRequest_WhenFileMissing()
    {
        var controller = new WorkRequestsBulkImportController(new StubBus(), new StubNumberGenerator());
    ...
    public async Task ShouldReturnOk_WhenRowsImported()
    {
        ...
        var controller = new WorkRequestsBulkImportController(bus, new StubNumberGenerator { Next = "WO-001" });
        ...
        var payload = ok.Value.ShouldBeOfType<WorkRequestBulkImportResponse>();
        payload.CreatedCount.ShouldBe(1);
        payload.Results.Count.ShouldBe(1);
        payload.Results[0].Success.ShouldBeTrue();
        payload.Results[0].WorkRequestNumber.ShouldBe("WO-001");
    ...
    private sealed class StubNumberGenerator : IWorkRequestNumberGenerator
    {
    ...
    private sealed class StubBus : IBus
    {
        ...
        public Task<TResponse> Send<TResponse>(IRequest<TResponse> request)
        {
            ...
            {
                SaveDraftCalls++;
                return Task.FromResult((TResponse)(object)new StateCommandResult(cmd.WorkRequest, "Save", "ok"));
            }
