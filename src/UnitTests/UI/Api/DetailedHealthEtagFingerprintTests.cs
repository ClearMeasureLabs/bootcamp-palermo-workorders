...
public class DetailedHealthEtagFingerprintTests
{
    ...
    private static DetailedHealthReport BuildReport(TimeProvider clock, double durationA, double durationB)
    {
        return HealthReportBuilder.FromEntries(
            clock,
            [
    ...
    public void FromReport_Should_ProduceEquivalentJson_When_OnlyVolatileFieldsDiffer()
    {
        var a = DetailedHealthEtagFingerprint.FromReport(
            BuildReport(new FixedUtcTimeProvider(new DateTime(2026, 1, 1, 0, 0, 0, DateTimeKind.Utc)), 1.5, 2.0));
        var b = DetailedHealthEtagFingerprint.FromReport(
            BuildReport(new FixedUtcTimeProvider(new DateTime(2026, 12, 31, 0, 0, 0, DateTimeKind.Utc)), 99.0, 0.1));
