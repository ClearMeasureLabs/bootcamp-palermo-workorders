...
public partial class Login : AppComponentBase
{
    ...
    /// <summary>
    /// Display-only formatting for the login member select. Uppercases the full name so all
    /// entries in the dropdown appear in capital letters; does not alter stored names.
    /// </summary>
    private static string GetLoginDropdownDisplayName(Employee employee)
    {
        return employee.GetFullName().ToUpperInvariant();
    }
