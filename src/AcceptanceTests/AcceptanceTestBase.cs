...
public abstract class AcceptanceTestBase
{
    ...
    protected async Task<WorkOrder> CreateAndSaveNewWorkOrder()
    {
        ...
        order.Number = null;
        return await CreateAndSaveNewWorkOrderFromOrder(order);
    }

    protected async Task<WorkOrder> CreateAndSaveNewWorkOrderFromOrder(WorkOrder order)
    {
        var testTitle = order.Title;
        var testDescription = order.Description;
        var testInstructions = order.Instructions ?? string.Empty;
    ...
    protected async Task<WorkOrder> AssignExistingWorkOrder(WorkOrder order, string username)
    {
        ...
        await Input(nameof(WorkOrderManage.Elements.Instructions), order.Instructions ?? string.Empty);
