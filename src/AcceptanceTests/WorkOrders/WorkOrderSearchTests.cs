...
public class WorkOrderSearchTests : AcceptanceTestBase
{
    ...
    public async Task ShouldDisplayUnassigned_ForNullAssignee()
    {
        ...
        // Filter by creator so only our unassigned order appears
        var creatorSelect = Page.Locator($"#{WorkOrderSearch.Elements.CreatorSelect}");
        await creatorSelect.SelectOptionAsync(creator.UserName);
        var searchButton = Page.Locator($"#{WorkOrderSearch.Elements.SearchButton}");
        await searchButton.ClickAsync();
        ...
        // Assert: assignee cell shows "Unassigned"
        var assigneeCell = Page.Locator(".grid-data tbody tr").First.Locator("td:nth-child(3)");
        await Expect(assigneeCell).ToContainTextAsync("Unassigned");
    }
}
