...
public class LoginTests : AcceptanceTestBase
{
    ...
    [Test]
    public async Task DuffmanCanLogin()
    {
        // Act: Go to home page
        await Page.GotoAsync("/");
        await Page.WaitForLoadStateAsync(LoadState.NetworkIdle);
        ...
        // Fill in username for duffman
        await Select(nameof(UI.Shared.Pages.Login.Elements.User), "duffman");
        await TakeScreenshotAsync(3);
        ...
        // Assert: Should be redirected to home and see welcome message
        var welcomeTextLocator = Page.GetByTestId(nameof(Logout.Elements.WelcomeText));
        await Expect(welcomeTextLocator).ToHaveTextAsync("Welcome duffman!");
    }
}