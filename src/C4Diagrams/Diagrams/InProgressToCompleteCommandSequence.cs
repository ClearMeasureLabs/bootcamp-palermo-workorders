...
public class InProgressToCompleteCommandSequence : SequenceDiagram
{
    protected override string Title => "InProgressToCompleteCommand Sequence";
    ...
    [
        ...
        Bound(alias: "cmdBound", label: "Command Pipeline",
            new Component(
                alias: "cmd",
                label: "InProgressToCompleteCommand",
                type: ComponentType.None,
                technology: "Command",
                description: ""
            ),
    ...
    [
        ...
        It("api") > It("bus") | "Send(InProgressToCompleteCommand)",
        It("bus") > It("mediator") | "Send(InProgressToCompleteCommand)",
        It("mediator") > It("cmdHandler") | "Handle(StateCommandBase)",
        It("cmdHandler") > It("cmd") | "Execute(StateCommandContext)",
        It("cmd") > It("order") | "CompletedDate = CurrentDateTime",
        It("cmd") > It("order") | "ChangeStatus(CurrentUser, date, Complete)",
        It("cmdHandler") > It("db") | "SaveChangesAsync()",
        It("cmdHandler") > It("mediator") | ("StateCommandResult", "Complete")
    ];
}
