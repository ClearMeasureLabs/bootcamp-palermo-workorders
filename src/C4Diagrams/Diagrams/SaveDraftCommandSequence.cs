...
public class SaveDraftCommandSequence : SequenceDiagram
{
    protected override string Title => "SaveDraftCommand Sequence";
    ...
    [
        ...
        It("api") > It("bus") | "Send(SaveDraftCommand)",
        It("bus") > It("mediator") | "Send(SaveDraftCommand)",
        It("mediator") > It("cmdHandler") | "Handle(StateCommandBase)",
        It("cmdHandler") > It("cmd") | "Execute(StateCommandContext)",
        It("cmd") > It("order") | "ChangeStatus(CurrentUser, date, Draft)",
        It("cmdHandler") > It("db") | "SaveChangesAsync()",
        It("cmdHandler") > It("mediator") | ("StateCommandResult", "Save")
    ];
}
