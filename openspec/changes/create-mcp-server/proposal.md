...
The ChurchBulletin system currently exposes its domain operations only through a Blazor UI and Web API. AI agents and LLM-powered clients have no standardized way to interact with work requests, employees, or other domain entities. The Model Context Protocol (MCP) is an emerging open standard for connecting AI assistants to external tools and data sources. Adding an MCP server enables any MCP-compatible AI client (Claude, Copilot, Cursor, etc.) to query and manage work requests programmatically, unlocking AI-assisted workflows without modifying the existing UI or API.
...
- MCP tools for work request operations: list, query by number, create, update status
- MCP tools for employee operations: list, query by username
- MCP resources for read-only reference data (work request statuses, roles)
...
- `mcp-work-request-tools`: MCP tools exposing work request query and mutation operations
