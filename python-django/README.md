...
`./scripts/private_build.sh` creates an isolated virtual environment, installs dependencies, deletes and recreates a clean build database, applies migrations, and runs Django tests. Add `RUN_ACCEPTANCE=1` to install Chromium and run browser acceptance. `pwsh -File ./PrivateBuild.ps1 -Acceptance` provides the PowerShell entry point expected by Windows Bootcamp users. Acceptance tests save a screenshot and Playwright browser video under `artifacts/`; set `HEADFUL=1` to show Chromium where a graphical desktop is available.
