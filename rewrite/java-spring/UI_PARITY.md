...
| `WorkOrderSearch` `/workorder/search` | `/workorder/search` | Partial | Creator, assignee, assigned-to-me, all-assigned, status, overdue, and clear-filter controls plus the work-order result columns are implemented. Click-to-sort headers remain open; FetchData, AI Agent, and Settings are documented but not ported. |
...
| New Work Order (Manage create mode) | `/workorder/manage?mode=Create` | Partial | Title, description, instructions, room, and due date can be created. Source voice input and exact field/action affordances are absent. |

## Parity verification in the Java acceptance run

The Playwright acceptance test captures Home, Search, Manage, and Counter screenshots at 1920×1080 and exercises login, create, creator/assignee search, assigned-to-me and all-assigned filtering, assignment, begin, completion, metadata attachment, and counter increment/reset. The workflow artifact must be reviewed after each UI change; this matrix is not a substitute for that visual comparison.
