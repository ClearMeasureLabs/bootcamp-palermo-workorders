...
| Playwright NUnit browser tests, `AcceptanceTests.ps1 -Headful` | Playwright for Java + JUnit 5; browser installation via Playwright CLI; headful mode via `headless=false` | Chromium create/assign flow and recorded video plus Remotion MP4 rendering are wired in CI; execution still needs verification |
