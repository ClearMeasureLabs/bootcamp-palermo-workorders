...
- Employee selection sign-in with uppercase names, a Timothy Lovejoy shortcut, logout, and server-side session identity that survives page reloads. This intentionally mirrors the source demo picker: it does not use passwords or represent a production identity provider.
...
| EF Core and ordered SQL scripts / Database console | Spring Data JPA + Flyway versioned migrations | Flyway V1 schema, V2 instructions-column guard, V3 employee/role and ownership schema, V4 attachment metadata schema, and Hibernate `validate` implemented for H2/PostgreSQL; original migration history and upgrade/rebuild tooling are not ported |
...
| Playwright NUnit browser tests, `AcceptanceTests.ps1 -Headful` | Playwright for Java + JUnit 5; browser installation via Playwright CLI; headful mode via `headless=false` | Browser acceptance covers employee login/logout, session persistence, work-order lifecycle and instructions/urgency, and attachment metadata display; recording/video run is pending for this update |
...
`.github/workflows/java-spring.yml` adds a Maven test/build gate. Java Maven tests, headful Chromium acceptance, and Remotion rendering passed in [run #35923838044](https://github.com/ClearMeasureLabs/bootcamp-palermo-workorders/actions/runs/35923838044) on commit `964a465f9a161445626736e83fd582f6d7c3999b`. This environment does not include Maven, PowerShell, Docker, or a browser installation, so verification was performed by GitHub Actions. Root .NET checks are separate and do not establish Java parity; the Java branch remains a partial rewrite. The Java workflow uploads the browser recording and rendered MP4 as an artifact.
