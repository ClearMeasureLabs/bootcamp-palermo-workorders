...
- Modify `WorkRequestSearch.razor` in `src/UI/Client/Pages/` to show `SkeletonTable` while search results are loading instead of a spinner
- Modify `WorkRequestManage.razor` in `src/UI/Client/Pages/` to show `SkeletonForm` while work request data is loading instead of a spinner
...
- Remove existing spinner/loading indicators from WorkRequestSearch and WorkRequestManage pages

## Capabilities
### New Capabilities
- Skeleton placeholder animations on WorkRequestSearch page during data load
- Skeleton placeholder animations on WorkRequestManage page during data load
...
- WorkRequestSearch replaces spinner with skeleton table during loading
- WorkRequestManage replaces spinner with skeleton form during loading

## Impact
- **UI.Shared**: New `SkeletonLine.razor`, `SkeletonTable.razor`, and `SkeletonForm.razor` components
- **UI.Client**: Modified `WorkRequestSearch.razor` and `WorkRequestManage.razor`, new CSS animation styles
...
### Acceptance Tests
- Navigate to WorkRequestSearch and verify the skeleton table appears with `data-testid="skeleton-table"` before results load, then is replaced by actual data
- Navigate to WorkRequestManage for an existing work request and verify the skeleton form appears with `data-testid="skeleton-form"` before data loads, then is replaced by the actual form
