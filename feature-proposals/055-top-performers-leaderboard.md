## Why
Recognizing top-performing employees motivates the team and promotes healthy competition. A leaderboard showing who has completed the most work requests in the current month gives visible credit to high achievers and helps managers identify reliable assignees.
...
- Add `LeaderboardHandler` in `src/DataAccess/Handlers/` filtering completed work requests by current month and grouping by assignee
...
## Capabilities
### New Capabilities
- Leaderboard component showing top employees ranked by completed work requests in the current calendar month
...
- `LeaderboardHandler` only counts work requests completed in the current month
- `LeaderboardHandler` calculates average completion time correctly per employee
- `LeaderboardHandler` returns empty list when no work requests completed in the current month
...
- Complete a work request and verify the assignee appears or moves up on the leaderboard on refresh
