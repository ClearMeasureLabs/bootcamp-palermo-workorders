## Why
External systems (facility management platforms, ticketing dashboards, mobile apps) need real-time notification when work request statuses change. Webhooks provide a standard, extensible integration point without requiring those systems to poll the API.
...
- Add `WebhookEvent` record in `src/Core/Model/` containing WorkRequestNumber, OldStatus, NewStatus, Timestamp, TriggeredBy
...
## Capabilities
### New Capabilities
- Fire HTTP POST webhook to configured URLs when work request status changes
...
- `WebhookEvent_Serialization_ContainsAllFields` - JSON payload includes work request number, old status, new status, timestamp
