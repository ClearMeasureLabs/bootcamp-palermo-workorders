## Why
Free-text room number entry leads to inconsistent data (e.g., "101", "Room 101", "Rm-101" all referring to the same location). A dropdown populated from a managed Room reference table ensures data consistency, enables accurate reporting by location, and reduces errors during work request creation.
...
- Replace free-text `RoomNumber` input with a `<select>` dropdown on the work request create and edit forms in `src/UI/Client/Pages/`
- Populate dropdown from `AllRoomsQuery` results, displaying "Building - Floor - Room Number"
- Update `WorkRequest.RoomNumber` to reference `Room.Number` (keep as string for backward compatibility)
...
- Room dropdown on work request create and edit forms replacing free-text input
- Rooms API endpoint for programmatic access to room list
- Room display format: "Building - Floor N - Room Number"

### Modified Capabilities
- Work request create and edit forms updated to use dropdown instead of text input for RoomNumber
...
- **src/UI/Client/Pages/** - Work request forms updated to use room dropdown
...
- `WorkRequest_RoomNumber_MatchesSelectedRoom` - Selecting room from dropdown sets correct RoomNumber value on work request

### Integration Tests
- `Room_SeededData_AccessibleViaQuery` - After migration, AllRoomsQuery returns seeded rooms
- `WorkRequest_WithRoomFromDropdown_PersistsCorrectRoomNumber` - Create work request selecting room from dropdown, verify persisted RoomNumber matches
- `RoomApi_ReturnsAllRooms_AsJson` - GET `/api/rooms` returns complete room list as JSON

### Acceptance Tests
- `CreateWorkRequest_RoomDropdown_ShowsAvailableRooms` - Log in, navigate to create form, click room dropdown, verify room options are displayed
- `CreateWorkRequest_SelectRoom_WorkRequestShowsCorrectRoom` - Select a room from dropdown, submit form, navigate to work request detail, verify room number displayed
- `EditWorkRequest_RoomDropdown_ShowsCurrentSelection` - Navigate to edit form for existing work request, verify dropdown shows currently assigned room as selected
