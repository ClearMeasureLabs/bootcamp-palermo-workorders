## Why
Navigating to the manage page for small edits like changing a title or reassigning a work request is slow for users managing many work requests. Inline editing in the search results table allows quick updates without leaving the list view, significantly improving efficiency.

## What Changes
- Modify `WorkRequestSearch.razor` in `src/UI/Client/Pages/` to support inline editing mode on table rows
...
- Add `UpdateWorkRequestInlineCommand` in `src/Core/Model/StateCommands/` for saving inline field changes (title, assignee)
- Add handler for `UpdateWorkRequestInlineCommand` in `src/DataAccess/Handlers/`
...
## Capabilities
### New Capabilities
- Click on a work request title in the search results table to edit it inline
...
- WorkRequestSearch table rows enhanced with inline edit capability for title and assignee columns

## Impact
- **Core**: New `UpdateWorkRequestInlineCommand` state command
...
- **UI.Client**: Modified `WorkRequestSearch.razor` with inline edit support
- **Database**: No schema changes required
- **Dependencies**: No new NuGet packages required

## Acceptance Criteria
### Unit Tests
- `UpdateWorkRequestInlineCommand_WithNewTitle_UpdatesTitle` - command handler persists title change
- `UpdateWorkRequestInlineCommand_WithNewAssignee_UpdatesAssignee` - command handler persists assignee change
...
### Integration Tests
- `UpdateWorkRequestInlineCommand_PersistsTitleChange_ToDatabase` - title change persists through save and reload
- `UpdateWorkRequestInlineCommand_PersistsAssigneeChange_ToDatabase` - assignee change persists through save and reload

### Acceptance Tests
- Navigate to WorkRequestSearch, click on a work request title cell with `data-testid="inline-edit-title"`, change the text, press Enter, and verify the new title is displayed
