## Why
Understanding the ratio of completed versus cancelled work requests over time reveals process health and helps identify periods where cancellation rates spike. A configurable time-period report empowers management to investigate root causes and improve completion rates.
...
- Add `CompletionRateHandler` in `src/DataAccess/Handlers/` filtering work requests by `CompletedDate` or status change date within the selected period
...
## Acceptance Criteria
### Unit Tests
- `CompletionRateHandler` calculates correct percentage when both completed and cancelled work requests exist
- `CompletionRateHandler` returns 100% when all work requests in the period are completed
- `CompletionRateHandler` returns 0% when all work requests in the period are cancelled
- `CompletionRateHandler` returns null or N/A when no completed or cancelled work requests exist in the period
...
- Date filtering correctly includes only work requests within the selected range
