## Why
Facility managers migrating from spreadsheet-based tracking or receiving bulk maintenance requests from building inspections need to import multiple work requests at once. CSV import eliminates tedious one-by-one data entry and enables batch onboarding of historical or external work request data.
...
- Add `CsvImportHandler` in `src/DataAccess/Handlers/` that parses CSV, validates each row, and creates draft work requests for valid rows
- Expected CSV columns: Title, Description, RoomNumber, CreatorUsername (header row required)
- Validate that CreatorUsername references an existing Employee with CanCreateWorkRequest permission
- Add file upload component on the work request list page in `src/UI/Client/Pages/` with "Import CSV" button
- Add `CsvImportController` in `src/UI/Api/` with `POST /api/workrequests/import` accepting multipart file upload
...
## Capabilities
### New Capabilities
- CSV file upload via UI "Import CSV" button on work request list page
...
- Bulk creation of draft work requests from valid CSV rows
...
- **src/UI/Client/Pages/** - File upload component and "Import CSV" button added to work request list page
...
- **Database** - No schema changes; creates work requests using existing schema

## Acceptance Criteria
### Unit Tests
- `CsvImport_ValidFile_CreatesWorkRequestsForEachRow` - CSV with 5 valid rows creates 5 draft work requests
...
- `CsvImport_CreatorWithoutPermission_ReportsRowError` - Row with creator lacking CanCreateWorkRequest role reports error
...
### Integration Tests
- `CsvImport_ValidFile_WorkRequestsPersistedInDatabase` - Import valid CSV, query database, verify all work requests exist with correct data
- `CsvImport_AllCreatedAsDraft_StatusIsDraft` - Import CSV, verify all created work requests have Draft status
...
### Acceptance Tests
- `CsvImport_UploadFile_ShowsImportResults` - Log in, navigate to work request list, click "Import CSV", upload valid file, verify success count displayed
- `CsvImport_InvalidRows_ShowsErrorDetails` - Upload CSV with invalid rows, verify error details displayed per failed row
- `CsvImport_ImportedWorkRequests_AppearInList` - Upload valid CSV, verify imported work requests appear in the work request list
