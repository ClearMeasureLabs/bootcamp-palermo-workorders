## Why
Maintenance teams coordinate through Slack and need immediate visibility when work requests change status. Automated Slack notifications reduce response time for newly assigned work and keep stakeholders informed of progress without requiring them to check the application.

## What Changes
- Add `ISlackNotificationService` interface in `src/Core/Interfaces/` with method `SendStatusChangeNotification(WorkRequest workRequest, WorkRequestStatus previousStatus)`
...
- Format Slack messages with work request number, title, old status, new status, and assignee

## Capabilities
### New Capabilities
- Automatic Slack message when a work request transitions between any statuses
- Configurable Slack webhook URL and channel via application settings
- Slack messages include work request number, title, previous status, new status, and assignee name
...
- `SlackNotification_MessageFormat_ContainsWorkRequestDetails` - Message body contains work request number, title, old status, and new status
...
### Integration Tests
- `SlackNotification_WorkRequestAssigned_PublishesNServiceBusMessage` - Assign a work request, verify NServiceBus message is published with correct data
- `SlackNotification_EndToEnd_DeliversToMockEndpoint` - Transition work request status, verify mock HTTP endpoint receives Slack webhook payload

### Acceptance Tests
- `SlackNotification_AssignWorkRequest_TriggersSlackMessage` - Assign a work request through the UI, verify mock Slack endpoint receives notification with correct work request number and assignee
- `SlackNotification_CompleteWorkRequest_TriggersSlackMessage` - Complete a work request through the UI, verify mock Slack endpoint receives notification with "Complete" status
