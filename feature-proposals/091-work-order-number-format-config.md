## Why
Different facilities use different numbering conventions for tracking maintenance requests. Making the work request number format configurable allows each deployment to match its existing numbering scheme, improving consistency with legacy systems and reducing confusion during migration.
...
- Add `NumberFormatSettings` configuration class in `src/Core/` bound to `appsettings.json` section `WorkRequestNumberFormat`
- Extend existing `IWorkRequestNumberGenerator` interface in `src/Core/Services/` (currently has `GenerateNumber()`) with an overload or update to accept format configuration
- Update existing `WorkRequestNumberGenerator` implementation in `src/Core/Services/Impl/` (currently generates 7-character uppercase GUID substring) to support configurable prefix prepended to the GUID-based number
...
## Capabilities
### New Capabilities
- Configurable work request number prefix (e.g., "WO-", "MNT-", "REQ-") prepended to the existing 7-character GUID-based number
...
- **src/Core/Services/IWorkRequestNumberGenerator.cs** - Extended existing interface (currently has `GenerateNumber()`)
- **src/Core/Services/Impl/WorkRequestNumberGenerator.cs** - Updated existing implementation (currently generates 7-char GUID substring)
...
- **src/UI/Server/appsettings.json** - New `WorkRequestNumberFormat` configuration section
...
- `NumberFormat_UpdateAndCreate_NewWorkRequestUsesNewFormat` - Update number format via admin page, create new work request, verify work request number matches new format
