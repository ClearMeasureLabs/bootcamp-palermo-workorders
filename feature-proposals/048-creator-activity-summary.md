## Why
Understanding creator activity patterns helps management identify high-volume requesters and assess whether work request quality correlates with creator behavior. A per-creator summary enables targeted training and process improvements for frequent requesters.
...
- Add `CreatorActivitySummaryHandler` in `src/DataAccess/Handlers/` grouping work requests by `Creator` and computing aggregates
...
- `CreatorActivitySummaryHandler` calculates average resolution time only from completed work requests
- `CreatorActivitySummaryHandler` handles creators with zero completed work requests gracefully
...
- Create and complete a work request, then verify the creator's statistics update on refresh
