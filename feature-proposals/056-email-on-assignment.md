## Why
Assignees need immediate awareness when a work request is assigned to them so they can begin planning and execution promptly. Email notification on assignment reduces response time and eliminates the need for manual follow-up by managers.
...
- Add `AssignmentEmailNotificationHandler` in `src/DataAccess/Handlers/` implementing `INotificationHandler<WorkRequestAssignedNotification>` from MediatR
- Add `WorkRequestAssignedNotification` to `src/Core/` as a MediatR notification published after `DraftToAssignedCommand` executes
...
## Capabilities
### New Capabilities
- Email sent to the assignee's `EmailAddress` when a work request transitions from Draft to Assigned
- Email contains: work request number, title, description, room number, and a link to view the work request
...
- `AssignmentEmailNotificationHandler` includes work request number and title in the email subject
- `AssignmentEmailNotificationHandler` includes work request details in the email body
...
### Integration Tests
- Assigning a work request triggers the notification handler with a stub email service
- Email service receives the correct parameters

### Acceptance Tests
- Assign a work request to an employee and verify an email is sent (using a test email interceptor or log verification)
- Verify the email content includes the work request number, title, and link
