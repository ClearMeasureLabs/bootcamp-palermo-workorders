...
Current local validation: `python manage.py check`, `python manage.py makemigrations --check --dry-run`, and `python manage.py test` pass (6 tests). Playwright Python installed, but Chromium installation failed in this sandbox because the browser archive download was truncated; the CI workflow installs Chromium from the runner and runs the browser acceptance sequence under Xvfb, captures `.webm` plus screenshot, renders a Remotion MP4, and uploads all three as `django-work-order-acceptance`. Green browser and Remotion results require the PR workflow to complete successfully.
