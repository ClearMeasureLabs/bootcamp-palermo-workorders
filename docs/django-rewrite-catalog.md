...
Current local validation: `python manage.py check`, `python manage.py makemigrations --check --dry-run`, and `python manage.py test` pass (23 tests). Playwright Python is installed, but Chromium is unavailable in this sandbox; the CI workflow installs Chromium on the runner and runs browser acceptance under Xvfb, captures the Django server log, `.webm`, and screenshot, renders a Remotion MP4, and uploads them as `django-work-order-acceptance`.
