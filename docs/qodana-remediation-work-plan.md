...
|-------|------:|---:|--------|
| P0 High correctness | #8842 | #8850 | Done |
| P1 UI HTML/nullable | #8843 | #8930 | Done |
| P2 dead code / namespaces | #8844 | #8931 | Done |
| P3 mechanical High | #8845 | #8932 | Done |
| P5 fail-gate | #8846 | #8934 | Done — hard gate ON |
| Residual usings + baseline refresh | #8933 | #8935 | Done |

**CI gate on master:** Qodana job has **no** `continue-on-error`; `qodana.yaml` has `failThreshold: 0`; scan uses `--baseline,qodana.sarif.json,--fail-threshold,0`. Check-run `Qodana (Community .NET)` is `success` on the tip above.

This document remains the historical categorization and sequencing record; execution tracked on the linked issues/PRs.
