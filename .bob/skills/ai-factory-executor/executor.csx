...
// ============================================================================
// Discovers issues labeled "AI Factory" in "Development" column, delegates to
// subagents, monitors PRs, and orchestrates parallel execution.
//
// Usage:
//   dotnet-script executor.csx
//   dotnet-script executor.csx -- --max-concurrent 3 --dry-run
//   dotnet-script executor.csx -- --org MyOrg --repo MyRepo
// ============================================================================
...
static class GitHub
{
    ...
    public static async Task<(string Org, string Repo)> GetRepoInfo(string org, string repo)
    {
        ...
        var match = Regex.Match(result.Output, @"github\.com[:/]([^/]+)/([^/\.]+)");
        if (!match.Success)
        {
    ...
    public static async Task<List<Issue>> GetEligibleIssues(string org, string repo)
    {
        ...
        var result = await RunCommand("gh", 
            $"issue list --repo {org}/{repo} --label \"AI Factory\" --state open --json number,title,body,url,labels,state --limit 100",
            captureOutput: true);
...
static class SubagentManager
{
    ...
    {
        var slug = Regex.Replace(issue.Title, @"[^a-zA-Z0-9]+", "-")
            .ToLower()
            .Substring(0, Math.Min(50, issue.Title.Length));
...
";
    ...
    public static Subagent StartSubagent(SubagentTask task, bool dryRun)
    {
        ...
        // TODO: Integrate with actual Task tool / subagent spawning mechanism
        // For now, this is a placeholder that would need to be replaced with:
        // - Bob Shell's Task tool invocation
        // - Or direct subprocess spawning with isolated context
        
        Console2.Info("Subagent spawning not yet implemented - requires Task tool integration");
    ...
    {
        // TODO: Implement actual status checking via Task tool
