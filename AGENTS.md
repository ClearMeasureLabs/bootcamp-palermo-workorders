...
- **Contract:** `src/UI/Server/Protos/workrequests.proto`. Generated C# is checked in under `src/UI/Server/Generated/Protos/` so Linux ARM64 CI avoids `Grpc.Tools` `protoc` (which can segfault on that platform). After changing the `.proto`, regenerate on an x64 machine with `Grpc.Tools` and replace those files.
...
- **Surface:** `workrequests.WorkRequests` — `Ping` (smoke) and `GetWorkRequestByNumber` (reads via `IBus` / `WorkRequestByNumberQuery`, same path as HTTP APIs).
...
var reply = await client.GetWorkRequestByNumberAsync(
    new GetWorkRequestByNumberRequest { Number = "WO-123" });
```
