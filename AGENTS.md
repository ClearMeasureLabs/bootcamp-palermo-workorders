...
**Always** lead user-visible status / progress updates with an absolute local timestamp (with timezone offset) and a relative age marker such as `(just now)`. Rule: `.cursor/rules/timestamped-status-updates.mdc` (also stored in the account Cursor rules directory for Jeffrey Palermo).
