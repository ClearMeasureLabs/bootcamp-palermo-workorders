...
`src/` contains NestJS health, demo authentication, employee directory, and work-order API endpoints; `sql.js` persists a local SQLite database and applies versioned migrations for due dates/instructions plus employee, role, and session records; `public/index.html` is a minimal real browser UI. Work-order APIs require a server-validated bearer session. The login screen preserves the source's employee picker and Timothy Lovejoy shortcut; in this prototype those users are seeded demo identities and login has no password, so it is not an identity provider. The server checks employee existence, expires sessions after 12 hours, and revokes them on logout. API behavior includes draft creation, list/detail, text/status/assignee filtering, room/instructions/due-date persistence, and assigned/completed timestamps. Transition authorization mirrors the source actor checks: creators assign/cancel, assignees begin/shelve/complete, and each transition requires the matching begin status. This prototype additionally enforces `CanCreateWorkOrder` on creation and `CanFulfillWorkOrder` for assignment targets as server-side hardening; those capability flags are also reflected in the UI. Due-date urgency is calculated at read time against the America/Chicago calendar day and is suppressed for completed/cancelled orders. `src/work-orders/work-orders.e2e-spec.ts` exercises these contracts against SQLite. `scripts/acceptance.mjs` starts the service, records a Playwright browser video, and runs a real browser flow through demo login, due-today display, assignment search, assignee login, completion, and logout. `scripts/private-build.mjs` is this prototype's private-build analogue: clean local SQLite environment, locked dependency install, TypeScript compile and automated test.
...
This prototype now implements the core lifecycle, work-order search/details, Chicago due-date urgency, employee selection sessions, actor checks, and metadata-only attachment registration/display. Employee directory and roles are seeded demo data with no administrative management, password authentication, external identity provider, or production-grade session-cookie/token protections. Attachment metadata does not upload or persist binary file contents. Search pagination/sorting, audit trail, bulk import, API key/rate-limit/idempotency contracts, gRPC, MCP, LLM/chat/speech, realtime notifications, distributed worker/outbox, telemetry/exporters, deployment/IaC, and most of the original test inventory are not ported. SQLite and demo identities keep the slice runnable without provisioned services. The existing .NET checks do not validate this prototype; use `typescript-nest-prototype.yml` to gate it. A complete rewrite requires porting and proving each inventory item and replacing the repository root build/deployment contract after acceptance of this prototype.
