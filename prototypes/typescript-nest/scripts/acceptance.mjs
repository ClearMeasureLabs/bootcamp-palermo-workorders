...
for (const file of ['acceptance-video.webm', 'work-order-search-1365x900.png', 'work-order-manage-1365x900.png', 'work-order-manage-1920x1080.png', 'work-order-create-1365x900.png']) rmSync(path.join(artifacts, file), { force: true });
...
try {
  ...
  await page.locator('#orders a[data-manage-id]').filter({ hasText: /^WO-/ }).last().click();
