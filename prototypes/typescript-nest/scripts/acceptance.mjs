...
for (const file of ['acceptance-video.webm', 'work-order-search-1365x900.png', 'work-order-manage-1920x1080.png']) rmSync(path.join(artifacts, file), { force: true });
...
try {
  ...
  await page.locator('#create summary').click();
  ...
  await page.locator('#orders details').filter({ hasText: acceptanceTitle }).locator('summary').click();
  const newOrder = page.locator('#orders article').filter({ hasText: acceptanceTitle });
  await newOrder.waitFor();
  await page.setViewportSize({ width: 1920, height: 1080 });
  await newOrder.scrollIntoViewIfNeeded();
