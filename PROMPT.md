...
- **Issue:** #7164 — Add a small 'Built with .NET' badge text to the About page
- **URL:** https://github.com/ClearMeasureLabs/bootcamp-palermo-workorders/issues/7164
- **Previous Status:** N/A
- **Current Status:** Development
- **Repository:** ClearMeasureLabs/bootcamp-palermo-workorders
- **Workflow Key:** issue:ClearMeasureLabs/bootcamp-palermo-workorders#7164
- **Occurred At:** 2026-07-23T00:37:30.0932601+00:00
...
6. **Launch the app for live preview on port 8080 — via `dotnet run` from the project directory**, the
   same way you would run it on a local workstation. Do NOT publish and do NOT pass `--configuration
   Release`: a Release/publish serve makes the Blazor WASM boot manifest reference dev-only modules
   (Hot Reload, .pdb symbols) that 404 behind the preview tunnel and crash the client. Default (Debug)
   `dotnet run` in Development serves a self-consistent asset set that renders:
   ```bash
   cd src/UI/Server
   ...
   that port. `--no-launch-profile` avoids the Windows-only LocalDB connection string in
   `launchSettings.json`. Re-launch after subsequent code changes so the preview reflects your work.
